...
- 敵・集落・NPC・勢力拠点・イベント背景・アイテム由来などのロアを作成/改修する際は、必ず `docs/lore/lore-rules.md` を先に参照する。
...
### セーブ/ロードの往復に消費フィールドを含める

セーブは辞書の**明示的なフィールド列挙**（`save_manager.gd` の `_serialize_map_nodes` 等）で行われるため、実行時状態に新しいキーを足しても自動では保存されない。保存漏れがあると、ロード直後だけ `Dictionary.get(key, default)` が default に落ち、**エラーなく機能が無効化される**（「宣言したデータフィールドは必ず消費側で読む」と同型の静かな劣化）。実例: マップノードの `faction`/`site` がセーブに含まれておらず、`site` を読む同行者ESCORT判定がロード後のランで一切成立しなかった。

- 実行時状態（`map_nodes` の要素、GameManager のラン変数等）に新しいフィールドを追加・参照するときは、`save_manager.gd` の**シリアライズとデシリアライズの両方**に同じキーを追加する。
- 「新機能がロード後のランでも動くか」を、消費側の `get(key, default)` が default に落ちないかで確認する。
...
`docs/design/gdd-*.md` の番号付きサブセクション（`27-1`, `27-5b` 等）に同一セッション内で複数回追記した結果、末尾へ足し続けたことでキャラクター単位の記述がファイル内で分断された（例: 元レイダーの内容が `27-1` / `27-10` / `27-15` の3箇所に分裂）。追記のたびに動作は正しくても、**採番と挿入位置の一貫性はエラーにならず静かに崩れる**——`status_effect` の `pass` 握りつぶしと同じ「レビューしないと気づけない」種類の劣化。
