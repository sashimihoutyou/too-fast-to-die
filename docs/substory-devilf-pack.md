...
> **実装依存（重要）**: この実例は §9 のメカニズム（QuestManager の強制発火・QuestData・ボス修飾チャネル）に依存する。
> これらは未実装のため、本書の `.tres` ブロックを `resources/events/` にそのまま落とすと
> **遅延イベントがランダム抽選に紛れて因果が壊れる**。実装が揃うまでは本書を「仕様」として保持し、
> live 投入はしないこと。
...
title = "デビルフの群れ"
body_text = "家畜と人をさらう変異狼の群れに、辺境の集落が脅えている。群れは一頭の巨大なアルファに率いられ、東のボス領域へ向かっているという。──死骸の周りの足跡をよく見ると、狼のものに混じって、ひどく小さな裸足の跡がいくつかある。"
required_act = 1
...
label = "亡骸を葬る"
requirement = ""
result_text = "毒は獲物を選ばなかった。死骸を漁っていた数人の子供が、冷たくなって倒れていた。あの小さな足跡の主たちだ。土を掘り、埋める。噂は荒野を伝っていく。"
...
title = "毒の行方"
body_text = "群れを仕留めた死骸の野に戻ると、狼以外のものまで倒れていた。隠れて生きていた戦災孤児の一団が、毒の回った肉を口にしたのだ。"
required_act = 1
...
1. **常時（全PC）**: 導入 `body_text` の「小さな裸足の跡」。遅延先で明示的に回収される。
2. **能力依存（PC/同行者）**: 調教師の獣読み・犬／密告者同伴は「**知り得た**」フラグを立てる。
   → 毒餌の**遅延カルマを加重**（知っていてやった罪）。クリック追加なしで、テレグラフ公平性を
   **キャラ非対称性に変換**する。
...
## 9. 実装依存（このサブストーリーが必要とするもの）

前ターンで「オプションB」とした機構。**未実装**。これが揃って初めて本書の `.tres` が live 投入可能。

1. `GameManager.total_nodes_visited: int`（`advance_node()` で単調増加、`advance_act()` でもリセットしない）。期限基準。
2. `scripts/data/quest_data.gd`（`QuestData`）＋ `autoload/quest_manager.gd`（`QuestManager`）。
   アクティブクエスト保持・`deadline = total_nodes_visited + node_limit`・hunt の撃破数追跡・アウトカム確定・遅延ペイロード装填。
3. **EVENTノードの強制発火フック**: `event_screen.gd::_pick_event()` の前段で、QuestManager が装填した
   ペイロードを最優先で返す。無ければ従来のランダム抽選にフォールバック。
   （現状は未遭遇プールからの純ランダムのため、これが無いと遅延イベントの因果が壊れる）
4. `EventChoiceData` への新規フィールド: `starts_quest: StringName` / `quest_outcome: StringName` / `sets_flag: StringName`。
   **追加したら必ず消費側（QuestManager / フラグ参照）も同時実装**（宣言だけの死蔵フィールドを作らない）。
5. **ボス修飾チャネル**: `CombatManager` がボス生成時にアウトカム/フラグを参照し、取り巻き数・HPを調整。
6. （任意）明示テレグラフを「情報画面→選択画面」の2段で見せたい場合、`EventData` に follow-up 参照を足す小拡張。
